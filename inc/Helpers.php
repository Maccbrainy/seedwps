 ...
 * @link https://github.com/Alecaddd/awps/issues/18 and
 * @link https://github.com/Alecaddd/awps/blob/master/inc/Helpers.php
 *
 * @package seedwps
 */
...
if (! function_exists('mix')) {
	...
	function mix($path, $manifestDirectory = '')
	{
		if (! $manifestDirectory) {
			//Setup path for standard SEEDWPS-Folder-Structure
			$manifestDirectory = "assets/public/";
		}
		static $manifest;
		if (! starts_with($path, '/')) {
			$path = "/{$path}";
...
if ( ! function_exists('assets') ) {
	/**
	 * Easily point to the assets public folder.
	 *
	 * @param  string  $path
	 */
	function assets($path)
	{
		...
		echo get_template_directory_uri() . '/assets/public/' . $path;
	}
}
...
if ( ! function_exists('svg') ) {
	...
	function svg($path)
	{
		...
		echo get_template_part('assets/public/svg/inline', $path . '.svg');
	}
}
